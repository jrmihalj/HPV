...
{
  // intercepts
  real<lower = 0, upper = 1> psi_initial; // initial occupancy probability: equal for all strains
  
  // params of fixed effects 
  vector[n_strains] betas[n_strains];
  real<lower = 0.0001> beta_scale;
...
model {
  ...
  //Priors: 
  //prior on psi (range is 0-1)
  psi_initial ~ uniform(0,1);
  
  // prior on fixed covariate effects
  //beta_scale -???
  for( i in 1:n_strains){
    for( j in 1:n_strains){
      betas[i,j] ~ double_exponential(0, beta_scale);
    }
  }
