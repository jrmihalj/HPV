data {
  ...
  vector[2] dir_prior;
  int visit[n];
  int complete_obs[n_complete_obs];
}

transformed data {
  row_vector[d] sign[n_complete_obs];
  row_vector[d] zero_vec;

  for (j in 1:d) {
    zero_vec[j] = 0;
    for (i in 1:n_complete_obs) {
      if (y[complete_obs[i], j] == 1) {
        sign[i, j] = 1;
      } else {
        sign[i, j] = -1;
      }
...
parameters {
  ...
  row_vector<lower=0>[d] abs_ystar[n_complete_obs];
...
transformed parameters {
  ...
  for (i in 1:n) {
    ...
    else{ // if observation is missing 
    	for(k in 1:d){
    		fixedef_phi[i,k] = 0;
    		fixedef_gam[i,k] = 0;
    	}
  ...
  for (j in 1:d) {
    for (i in 1:n) {
      ...
      if(visit[i] > 1) { // code missing visits as -1 
