...
p3 <- qplot(x=psi1, y=psi2, data=df, geom="point", xlim=c(0,1), ylim=c(0,1),
...
<<fig=T, echo=F, height=3, width=9>>=
library(ggplot2)
n1 <- subset(corr.phi, Strain == 1)[,2:4]
n2 <- subset(corr.phi, Strain == 2)[,2:4]
...
<<fig=T, echo=F, height=3, width=9>>=
library(ggplot2)
n1 <- subset(corr.both, Strain == 1)[,2:4]
n2 <- subset(corr.both, Strain == 2)[,2:4]
...
a1 <- subset(among.phi, Strain == 1)
a2 <- subset(among.phi, Strain == 2)

among <- cbind(a2$lphi, a1$lphi, a1[,6])
colnames(among) <- c("phi2", "phi1", "Pat")
among <- data.frame(among)
among <- subset(among, Pat < 15)
...
b1 <- subset(both.phi, Strain == 1)
b2 <- subset(both.phi, Strain == 2)

both <- cbind(b2$lphi, b1$lphi, b1[,6])
colnames(both) <- c("phi2", "phi1", "Pat")
both <- data.frame(both)
both <- subset(both, Pat <= 15)
