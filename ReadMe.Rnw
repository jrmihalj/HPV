...
\begin{center}
<<echo=FALSE,fig=TRUE,>>=
mod.time <- glm(Y ~ X.time.vals * factor(Strain), data = Occ, family="binomial")
preds <- predict(mod.time)

plot(x=NA,y=NA, xlim=c(-3,3), ylim=c(0,1.2), xlab="Covariate", ylab="Occupancy")
points(Y ~ X.time.vals, data = Occ[Strain==1,], col="orange", pch=1)
points(Y+.05 ~ X.time.vals, data = Occ[Strain==2,], col="black", pch=1)
points(AntiLogit(preds[Strain==1]) ~ X.time.vals[Strain==1], col="orange", pch=19, cex=.25)
points(AntiLogit(preds[Strain==2]) ~ X.time.vals[Strain==2], col="black", pch=19, cex=.25)
legend(x=-3, y=.9, 
       legend=c(paste("Strain1, True Beta:",round(beta.time[1],2)), paste("Strain2, True Beta:",round(beta.time[2],2))), 
       col=c("orange","black"), pch=c(1,1), cex=.75)
@
\end{center}
