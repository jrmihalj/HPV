...
k = Sys.getpid()
print(k)
dbResultsFilename <- paste0("./output/fit_sim_chain_",k,".sqlite")

# Load the data:
load("data/test_data_10_strains_1500_pat.rda")
...
m_fit <- stan(fit = test,
              data = stan_d,
              init = inits_f,
              chains = 1, iter = 5000, warmup = 2000, thin = 3,
              pars = params,
...
posts = extract(m_fit, pars = params[1:20])
